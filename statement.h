...
public:
    ...
    std::vector<std::string>& GetVector () {return dot_;}
    
    runtime::ObjectHolder Execute(runtime::Closure& closure, runtime::Context& context) override;
private:

    std::vector<std::string> dot_;
};
...
public:
    ...
    runtime::ObjectHolder Execute(runtime::Closure& closure, runtime::Context& context) override;
private:
    const runtime::Class* class_first_ptr_;
    std::unique_ptr<runtime::ClassInstance> class_new_ptr_;  
    std::vector<std::unique_ptr<Statement>> args_ {};
    // runtime::Class new_class_;
};
...
public:
    BinaryOperation(std::unique_ptr<Statement> lhs, std::unique_ptr<Statement> rhs) 
                        ...
                        , rhs_(std::move(rhs))
                        {}
    Statement* GetArgumentLhs () { return lhs_.get(); }
    Statement* GetArgumentRhs () { return rhs_.get(); }
private:
...
public:
    ...
    explicit Compound(Args&&... args) 
                    //: args_(Construct<std::unique_ptr<Statement>>(args)...)  // F(H(args)...);
                    {
    ...
    void MakeVectorArgs(T&& head, Args&&... args) {
        //auto temp = std::forward<T>(head);
        args_.push_back(std::move(head));
        if constexpr (sizeof...(args) != 0) {
            MakeVectorArgs(args...);  // Рекурсивно выводим остальные параметры
        }
    }
