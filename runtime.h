...
public:
    ...
    void Print(std::ostream& os, Context& context) override;
    std::vector<Method>* GetVectMethods () {return &methods_;}
    const std::vector<Method>* GetVectMethods () const {return &methods_;}
    //Closure* GetClosure () {return &map_;}
    // const Closure* GetClosure () const {return &map_;}
    // void GetHoldThis () { map_["self"] = ObjectHolder::Share(*this);}

private:
    std::string name_{};
    std::vector<Method> methods_{};
    const Class* parent_ = nullptr;
    //mutable Closure map_;
    //ObjectHolder hold_this = ObjectHolder::Share(*this);
};
